...
const Alunos = ({ data, selected, setSelected }: AlunosProps) => {
    ...
    return (
        <Card.Group style={{ height: 500, overflow: "auto" }}>
            {data.map((item, key) => <Aluno result={item.stations.filter((item) => item.result).length > 0} active={selected.includes(key)} onClick={handleClick(key)} id={key} key={key} name={"Aluno " + key} />)}
        </Card.Group>
    )
}
